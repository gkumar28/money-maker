...
public class TickAggregator {
    ...
    private final DoubleAccumulator low = new DoubleAccumulator(Math::min, Double.POSITIVE_INFINITY);
    ...
    public void updateHigh(double price) { high.accumulate(price); }

    public void updateLow(double price) {
        low.accumulate(price);
    }

    public void updateClose(double price) { close.set(price); }

    public void updateVolume(double v) {
        volume.add(v);
    }
